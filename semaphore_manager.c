...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int if_exist=get_semaphore_object_ID(ownerEnvID,semaphoreName);

	if(if_exist==E_SEMAPHORE_NOT_EXISTS)
	{
       struct Semaphore *semaphore=NULL;
       if_exist=allocate_semaphore_object(&semaphore);
       if(if_exist!=E_NO_SEMAPHORE)
       {
       semaphore->ownerID=ownerEnvID;
       strcpy(semaphore->name,semaphoreName);
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	if (semaphores[iD].value<0){
	    ...
	    LIST_INSERT_HEAD(&semaphores[iD].env_queue,myenv);
	    curenv=NULL;
	    fos_scheduler();
	}
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if (semaphores[iD].value<=0){
		...
		myenv=LIST_LAST(&semaphores[iD].env_queue);
		LIST_REMOVE(&semaphores[iD].env_queue,myenv);
		LIST_INSERT_HEAD(env_ready_queues,myenv);
