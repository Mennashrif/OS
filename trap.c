...
void check_stack(struct Env * curenv, uint32 fault_va);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
  ...
  uint32 *dir_table=curenv->env_page_directory;
  uint32 index=curenv->page_last_WS_index;

	if(isPageReplacmentAlgorithmFIFO()){


		 address=curenv->ptr_pageWorkingSet[index].virtual_address;
		 page_table=NULL;
		 p_index=PTX(address);
		 get_page_table(dir_table, (void*)address,&page_table);
		if(curenv->ptr_pageWorkingSet[index].empty==0)
		{
            if((page_table[p_index]&PERM_MODIFIED)==PERM_MODIFIED){
        	uint32 *ptr_frame_info1;
        	struct Frame_Info* m;
        	m=get_frame_info(dir_table,(void*)address,&ptr_frame_info1);
        	pf_update_env_page(curenv,(void*)address,m);
        }
            unmap_frame(dir_table,(void*)address);
		}
		struct Frame_Info *ptr_frame_info;
	    allocate_frame(&ptr_frame_info);
	    map_frame(dir_table,ptr_frame_info,(void*)fault_va,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
	    curenv->ptr_pageWorkingSet[index].empty=0;
        curenv->ptr_pageWorkingSet[index].virtual_address=fault_va;
			if(index==curenv->page_WS_max_size-1)
				index=0;
			else
				index++;

		  curenv->page_last_WS_index=index;
		  check_stack(curenv,fault_va);
	}
	 ...
	 if(isPageReplacmentAlgorithmCLOCK()){
		...
		while(1){
			...
			p_index=PTX(address);
			get_page_table(dir_table, (void*)address,&page_table);
			if((page_table[p_index]&PERM_USED)==PERM_USED)
			{
				page_table[p_index]=page_table[p_index]&~PERM_USED;
			}
			else{
		      if((page_table[p_index]&PERM_MODIFIED)==PERM_MODIFIED){
				 uint32 *ptr_frame_info1;
		...
		struct Frame_Info *ptr_frame_info;
	    allocate_frame(&ptr_frame_info);
	    map_frame(dir_table,ptr_frame_info,(void*)fault_va,PERM_USED|PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
	    curenv->ptr_pageWorkingSet[index].empty=0;
    	curenv->ptr_pageWorkingSet[index].virtual_address=fault_va;
		...
		if(curenv->ptr_pageWorkingSet[index].empty==0){
		   ...
		   uint32 tmp= env_page_ws_get_time_stamp(curenv,0);
		   uint32 check;
		   index=0;
		for(uint32 i=1;i<curenv->page_WS_max_size;i++){
		    check=env_page_ws_get_time_stamp(curenv,i);
			if(check<tmp)
			{
				tmp=check;
			    index=i;
		...
		get_page_table(dir_table, (void*)address,&page_table);
	      if((page_table[p_index]&PERM_MODIFIED)==PERM_MODIFIED){
			 ...
			 m=get_frame_info(dir_table,(void*)address,&ptr_frame_info1);
			 pf_update_env_page(curenv,(void*)address,m);
			 page_table[p_index]=page_table[p_index]&~PERM_MODIFIED;

			 }
		unmap_frame(dir_table,(void*)address);
		}

		env_page_ws_set_time_stamp(curenv,index);
		  struct Frame_Info *ptr_frame_info;
		  allocate_frame(&ptr_frame_info);
		  map_frame(dir_table,ptr_frame_info,(void*)fault_va,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
		  curenv->ptr_pageWorkingSet[index].empty=0;
